<?php
namespace App\Libs\UpFtp;

use App\Libs\UpFtp\UpFtp;

class Upload extends UpFtp{

    public function __construct($FTP_HOST,$FTP_PORT,$FTP_USER,$FTP_PASS) {
        parent::__construct($FTP_HOST,$FTP_PORT,$FTP_USER,$FTP_PASS);
    }
    ...
    private function _uploadImg($path, $file, $pid = NULL, $autoMkdir = TRUE) {
        //加载配置文件
        $imageUpload = config('UpFtp.imageUpload');
    ...
    public function uploadFile($path, $file, $pid = NULL, $autoMkdir = TRUE){
        //加载配置文件
        $videoUpload = config('UpFtp.videoUpload');
        $imageUpload = config('UpFtp.imageUpload');
        $file = $file['file'];
